...
class Test_entNet_Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.params = nn.ParameterDict({
            ...
            # shared parameters
            'X': nn.Parameter(nn.init.normal_(torch.empty(X_size, requires_grad=True, dtype=torch.float, device=self.device, names=('x','x2')), mean=0.0, std=0.1)),
            'Y': nn.Parameter(nn.init.normal_(torch.empty(Y_size, requires_grad=True, dtype=torch.float, device=self.device, names=('y','y2')), mean=0.0, std=0.1)),
            'Z': nn.Parameter(nn.init.normal_(torch.empty(Z_size, requires_grad=True, dtype=torch.float, device=self.device, names=('z','z2')), mean=0.0, std=0.1)),
...
fake_sentence = nn.init.normal_(torch.empty(input_size, dtype=torch.float, device='cuda'), mean=0.0, std=0.1)
